...
# Define the port on whis you want to connect 
port = 12345			
...
def encrypt(pt, rk_bin, rk):
    ...
    for i in range(0, 16):
        ...
        for j in range(0, 8):
            ...
            col = bin_dec(int(xor_res[j * 6 + 1] + xor_res[j * 6 + 2] + xor_res[j * 6 + 3] + xor_res[j * 6 + 4]))
            val = sbox[j][row][col]
    ...
    cipher_text = permute(final, fperm, 64)
    return cipher_text

cipher_text = bin_hex(encrypt(pt, rk_bin, rk))
print("Cipher Text : ", cipher_text)


s.send(cipher_text.encode())
print("Cipher Text sent to the Server!")
